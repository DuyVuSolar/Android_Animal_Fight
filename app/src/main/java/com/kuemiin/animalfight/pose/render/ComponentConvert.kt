package com.kuemiin.animalfight.pose.render
...
object ComponentConvert {
    ...
    fun convert(context: Context, component: Component, dir: String) {
        ...
        if(paths.isNotEmpty()){
            if(paths.first().contains("Animal")){
                val sortedFilenames = paths.sortedWith(compareBy {
                    it.substringAfter("Animal_").substringBefore(".webp").toInt()
                })
                component.resources = sortedFilenames
            }else{
