package com.kuemiin.animalfight.ui.fragment.camera
...
import com.kuemiin.animalfight.BaseApplication.Companion.effectFilter
import com.kuemiin.animalfight.R
import com.kuemiin.animalfight.base.BaseFragment
import com.kuemiin.animalfight.databinding.FragmentCameraBinding
...
import com.kuemiin.animalfight.ui.fragment.camera.CameraViewModel.Companion.TYPE_TIMER_ZERO
import com.kuemiin.animalfight.ui.fragment.camera.canvas.CanvasTiktokFileFilter.Companion.totalAnimals
import com.kuemiin.animalfight.ui.fragment.camera.render.CongratRender
import com.kuemiin.animalfight.ui.fragment.camera.render.DegreeRender
import com.kuemiin.animalfight.ui.fragment.camera.render.QuestionQuizRender
import com.kuemiin.animalfight.ui.fragment.camera.render.RandomAnimalRender
import com.kuemiin.animalfight.ui.fragment.camera.render.RectAnimalRender
import com.kuemiin.animalfight.ui.fragment.camera.render.TiktokVoiceRender
import com.kuemiin.animalfight.ui.fragment.camera.render.Timer321Render
import com.kuemiin.animalfight.ui.fragment.processing.ProcessingFragment
...
import com.kuemiin.animalfight.utils.loge
...
class CameraFragment : BaseFragment<FragmentCameraBinding>() {
    ...
    companion object {
        fun newBundle(effect: String, language : Language? = null) = bundleOf().apply {
            putString(Constant.KEY_EXTRA_DATA, effect)
            putString(Constant.KEY_EXTRA_LANGUAGE, language?.alpha2 ?: "us")
        }
    }
...
//        }
    ...
    override fun setUp() {
        ...
        viewModel.language = arguments?.getString(Constant.KEY_EXTRA_LANGUAGE, "us") ?: "us"
        ...
        viewModel.mCurrentCameraId = if (viewModel.isFrontCamera.get()) Camera.CameraInfo.CAMERA_FACING_FRONT else Camera.CameraInfo.CAMERA_FACING_BACK
                    ...
                    CameraViewModel.Event.HandleInitFilter -> {
                        ...
                        if (!viewModel.isInitingFilter.get()) {
                            ...
                            if (mTiktokVoiceRender1 == null && viewModel.typeFilter == EFFECT_TIKTOK_VOICE) {
                                ...
                                mTimer321Render?.setPosition(xQuestion.toInt(), previewHeight - yQuestion.toInt())
                            ...
                            if (viewModel.typeFilter == EFFECT_SING_A_SONG) {
                                xQuestion = previewWidth / 2f
                                yQuestion = previewHeight / 3f
                            ...
                            }else if (viewModel.typeFilter == EFFECT_ANIMAL_EFFECT) {
                                val widthRect = (previewWidth * 0.85f)
                                mRectAnimalRender = RectAnimalRender(requireContext(),
                                    widthRect.toInt(),
                                    viewModel.typeFilter
                                )
                                mRenderPipeline?.addFilterRender(mRectAnimalRender)
                                xQuestion = previewWidth / 2f
                                yQuestion = previewHeight / 3f
                                mRectAnimalRender?.setPosition(xQuestion.toInt(), yQuestion.toInt())
                                mRectAnimalRender?.currentQuiz = 0


                                val widthAnimal = (widthRect * 0.225f).toInt()
                                val spaceAnimal = (widthRect * 0.02f).toInt()
                                mAnimalRender1 = RandomAnimalRender(requireContext(), widthAnimal, viewModel.typeFilter)
                                mAnimalRender2 = RandomAnimalRender(requireContext(), widthAnimal, viewModel.typeFilter)
                                mAnimalRender3 = RandomAnimalRender(requireContext(), widthAnimal, viewModel.typeFilter)
                                mAnimalRender4 = RandomAnimalRender(requireContext(), widthAnimal, viewModel.typeFilter)
                                mAnimalRender5 = RandomAnimalRender(requireContext(), widthAnimal, viewModel.typeFilter)
                                mAnimalRender6 = RandomAnimalRender(requireContext(), widthAnimal, viewModel.typeFilter)
                                mAnimalRender7 = RandomAnimalRender(requireContext(), widthAnimal, viewModel.typeFilter)
                                mAnimalRender8 = RandomAnimalRender(requireContext(), widthAnimal, viewModel.typeFilter)
                                ...
                                var xAnimal = previewWidth - widthRect + (2.5f * spaceAnimal)
                                var yAnimal = yQuestion - 1.5 * spaceAnimal - widthAnimal
...
//        viewModel.cameraX?.cameraControl?.enableTorch(viewModel.isAllowOnFlash())
    ...
    private var mRectAnimalRender: RectAnimalRender? = null
    private var totalTimeDelayNextAnimal = 5270L//
    private var mAnimalRender1: RandomAnimalRender? = null
    private var mAnimalRender2: RandomAnimalRender? = null
    private var mAnimalRender3: RandomAnimalRender? = null
    private var mAnimalRender4: RandomAnimalRender? = null
    private var mAnimalRender5: RandomAnimalRender? = null
    private var mAnimalRender6: RandomAnimalRender? = null
    private var mAnimalRender7: RandomAnimalRender? = null
    private var mAnimalRender8: RandomAnimalRender? = null
    private var timeDelayNextAnimal = totalTimeDelayNextAnimal / 8 / 2
...
//endregion
    ...
    private val mStateCallback: CameraDevice.StateCallback = object : CameraDevice.StateCallback() {
        override fun onOpened(camera: CameraDevice) { // Open the camera
            ...
            filemp4 = File(requireContext().filesDir, System.currentTimeMillis().toString() + ".mp4")
    ...
    private fun startRenderTiktokVoice() {
            ...
            viewModel.viewModelScope.launch(Dispatchers.IO) {
                if (viewModel.isRecording.get()) {
                    ...
                    if (random == 0) {//0 ghe quay lai ti le 1/6
                        ...
                        val randomCase =
                            Random.nextInt(1, 6) // 1, 2, 3,4 se quay lai, 5 : khong quay lai
                        if (randomCase >= 5) {//3 ghe quay lai, case nay it nhat 1/6
                            listOf(
                                mTiktokVoiceRender1,
                                mTiktokVoiceRender2,
                                mTiktokVoiceRender3
                            ).shuffled()//1 ghe quay lai, 2 ghe quay lai
                            ...
                            if (randomCase in 1..3) {//1 ghe quay lai, case nay nhieu nhat, 50%
                                listOf(
                                    ...
                                    mTiktokVoiceRender2,
                                    mTiktokVoiceRender3
                                ).shuffled().first()?.currentQuiz = Random.nextInt(1, 3)
                                delay(4000)
                            } else {//2 ghe quay lai, case nay 1/6
                                listOf(mTiktokVoiceRender2, mTiktokVoiceRender3).shuffled().forEach {
                                    it?.currentQuiz = Random.nextInt(1, 3)
                                }
                                delay(2000)
                            }
    ...
    private fun startRenderAnimal()  = viewModel.viewModelScope.launch{
        var indexPageAnimal = 0
        if (viewModel.typeFilter == EFFECT_ANIMAL_EFFECT) {
            delay(3500)
            val listAnimals = listOf(
                mAnimalRender1, mAnimalRender2, mAnimalRender3, mAnimalRender4,
                mAnimalRender5, mAnimalRender6, mAnimalRender7, mAnimalRender8
            )
            mRectAnimalRender?.currentQuiz = indexPageAnimal

            viewModel.viewModelScope.launch(Dispatchers.IO) {
                while (viewModel.isRecording.get()) {
                    delay(totalTimeDelayNextAnimal / 2)
                    indexPageAnimal++
                    if(indexPageAnimal < 10) {
                        mRectAnimalRender?.currentQuiz = indexPageAnimal
                    }else{
                        indexPageAnimal = 0
                        mRectAnimalRender?.currentQuiz = indexPageAnimal
                    ...
                    listAnimals.forEach {animalRender ->
                        animalRender?.let {
                            it.currentQuiz = Random.nextInt(0,totalAnimals)
                        }
                    }
                    delay(totalTimeDelayNextAnimal / 2)
                }
            ...
            viewModel.viewModelScope.launch(Dispatchers.IO) {
                ...
                while (viewModel.isRecording.get()) {
                    listAnimals.forEach {animalRender ->
                        animalRender?.let {
                            it.currentQuiz = if(it.currentQuiz >= totalAnimals) it.currentQuiz - totalAnimals else it.currentQuiz
                        }
                    }
//
                    listAnimals[indexAnimal]?.let {
                        it.currentQuiz = it.currentQuiz + totalAnimals
                    }
                    indexSelect++
                    indexAnimal++
                    if (indexAnimal >= 8) {
