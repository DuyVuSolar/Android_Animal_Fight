...
import com.kuemiin.animalfight.base.BaseAdapter
import com.kuemiin.animalfight.base.BaseFragment
import com.kuemiin.animalfight.databinding.FragmentLanguageBinding
...
class LanguageFragment : BaseFragment<FragmentLanguageBinding>() {
    ...
    private val mAdapterLanguage by lazy {
        BaseAdapter<Language>(layoutInflater, R.layout.item_sing_along_language).apply {
            viewmodel = viewModel
        }
    }
    ...
    private fun setUpEvent() {
        lifecycleScope.launchWhenStarted {
            viewModel.homeEvent.collect { event ->
                when (event) {
                    LanguageViewModel.Event.OnClickBack -> {
                    ...
                    LanguageViewModel.Event.OnNavigateUp -> {
                        navigateUp()
                    }

                    LanguageViewModel.Event.OnClickLetStart -> {
                        val handle = {
                    ...
                    LanguageViewModel.Event.NavigateOnClickLetStart -> {
                        requireContext().logFirebaseEvent("click_sing_animal_let_start")
                        navigateTo(R.id.action_languageFragment_to_cameraFragment, CameraFragment.newBundle(Bitmap2025QuizFilter.EFFECT_SING_A_SONG, viewModel.listLanguage.first { it.isSelected }))
                    }

                    is LanguageViewModel.Event.OnClickItemLanguage -> {
