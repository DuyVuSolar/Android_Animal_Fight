...
class CameraViewModel @Inject constructor(
    ...
    var startTime = 0L
    var timeLimit = ObservableInt(MAX_TIME_RECORD_60s)
    ...
    var language = "us"
    var mCanvasFilter = CanvasTiktokFileFilter()
    var mCanvasAnimalFilter = CanvasAnimalFileFilter()
    ...
    //region applayer -> play with case animal
    ...
    fun initAudioAnimEffect() = viewModelScope.launch{
        if(typeFilter != EFFECT_ANIMAL_EFFECT) return@launch
        delay(200)
        val fileRaw = R.raw.audio_effect_animal_challage
        appPlayer.initUri(RawResourceDataSource.buildRawResourceUri(fileRaw), true)
        appPlayer.setRepeatMode(SimpleExoPlayer.REPEAT_MODE_ALL)
    }
    ...
    private fun setCanvasQuiz(){
        ...
        if(typeFilter == EFFECT_ANIMAL_EFFECT) {
            mCanvasAnimalFilter.setAnimalAndCanvas(context, listQuiz, typeFilter){
            ...
            mCanvasFilter.setQuestionTiktokAndCanvas(context, listQuiz,  language){
                isInitingFilter.set(false)
                if(cameraOpened){//voi truong hop camera mo truoc khi handle bitmnap
                    setResolution()
                    handleInitFilter()
    ...
    fun getListQuizQuestion(): ArrayList<QuizModel> {
        val listEffect = arrayListOf<EffectModel>().apply {
            addAll(Constant.getAllListMath(language))
        }
