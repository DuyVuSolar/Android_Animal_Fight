...
object NotchUtils {
    ...
    private fun Context?.hasNotchAtXiaoMi(): Boolean {
        ...
        if (Build.MANUFACTURER.lowercase().contains("xiaomi")) {
            try {
                ...
                val aClass = classLoader?.loadClass(com.kuemiin.animalfight.utils.NotchUtils.SYSTEM_PROPERTIES)
                val method = aClass?.getMethod(
                    "getInt", String::class.java,
                    Int::class.javaPrimitiveType
                )
                result = method?.invoke(aClass,
                    com.kuemiin.animalfight.utils.NotchUtils.NOTCH_XIAO_MI, 0) as Int
    ...
    fun Context?.hasNotchAtHuaWei(): Boolean {
        ...
        if (Build.MANUFACTURER.lowercase().contains("huawei")) {
            try {
                val classLoader = this?.classLoader
                val aClass = classLoader?.loadClass(com.kuemiin.animalfight.utils.NotchUtils.NOTCH_HUA_WEI)
    ...
    private fun Context?.hasNotchAtVIVO(): Boolean {
        ...
        if (Build.MANUFACTURER.lowercase().contains("vivo")) {
            try {
                val classLoader = this?.classLoader
                val aClass = classLoader?.loadClass(com.kuemiin.animalfight.utils.NotchUtils.NOTCH_VIVO)
    ...
    private fun Context?.hasNotchAtOPPO(): Boolean {
        if (Build.MANUFACTURER.lowercase().contains("oppo")) {
            return try {
                this?.packageManager?.hasSystemFeature(com.kuemiin.animalfight.utils.NotchUtils.NOTCH_OPPO) ?: false
            } catch (ignored: Exception) {
                false
            }
        }
    ...
    private fun Context?.hasNotchAtLenovo(): Boolean {
        if (Build.MANUFACTURER.lowercase().contains("lenovo")) {
            val resourceId = this?.resources?.getIdentifier(com.kuemiin.animalfight.utils.NotchUtils.NOTCH_LENOVO, "bool", "android") ?: 0
            if (resourceId > 0) {
    ...
    private fun Context?.hasNotchAtMeiZu(): Boolean {
        if (Build.MANUFACTURER.lowercase().contains("meizu")) {
            return try {
                val clazz = Class.forName(com.kuemiin.animalfight.utils.NotchUtils.NOTCH_MEIZU)
